...
/** Transform a StoredEvent into conversation messages. */
function eventToMessages(event: StoredEvent): ConversationMessage[] {
  ...
  switch (event.type) {
      ...
      return [
        {
          ...
          content: `Task created: ${p.title as string}`,
          metadata: { intent: p.intent as string, agentId: p.agentId as string },
        },
      ]
    case 'TaskStarted':
      return [{ ...base, id: `${event.id}-started`, role: 'system', kind: 'status', content: 'Agent started working' }]
    case 'TaskCompleted':
      return [{ ...base, id: `${event.id}-done`, role: 'assistant', kind: 'text', content: (p.summary as string) || 'Task completed.' }]
    case 'TaskFailed':
      return [{ ...base, id: `${event.id}-fail`, role: 'system', kind: 'error', content: `Task failed: ${p.reason as string}` }]
    case 'TaskCanceled':
    ...
    case 'TaskInstructionAdded':
      return [{ ...base, id: `${event.id}-inst`, role: 'user', kind: 'instruction', content: p.instruction as string }]
    case 'UserInteractionRequested':
      return [{
        ...
        role: 'assistant',
        kind: 'interaction',
        content: p.purpose as string,
        metadata: { interactionId: p.interactionId as string, kind: p.kind as string },
      }]
    case 'UserInteractionResponded':
      return [{
        ...
        role: 'user',
        kind: 'interaction',
        content: (p.inputValue as string) || (p.selectedOptionId as string) || 'Response submitted',
        metadata: { interactionId: p.interactionId as string },
      }]
    default:
      return []
  }
